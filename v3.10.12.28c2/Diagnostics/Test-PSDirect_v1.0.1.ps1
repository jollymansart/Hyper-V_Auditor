...
Function Test-PSDirect
{
    ...
    $credLookup = @{}       # Domain creds: key = domain FQDN (e.g. 'ohdc.com')
    $localCreds = @{}       # Local creds: key = synthetic (e.g. 'local:windows', 'local:linux')
    ...
    if (-not $CredentialPaths) {
        ...
        if (Test-Path $configPath) {
            ...
            foreach ($credDef in $config.Credentials) {
                # Must have StoredPath and file must exist
                if (-not $credDef.StoredPath -or -not (Test-Path $credDef.StoredPath)) {
                    if ($credDef.Name) {
                        Write-Status $credDef.Name "MISSING" "$($credDef.StoredPath) not found" Yellow
                    }
                    continue
                }
                ...
                try {
                    ...
                    Write-Status ($credDef.Name) "FAIL" "Cannot load $($credDef.StoredPath)" Red
                    continue
                }

                # Classify: Local vs Domain
                $credType = if ($credDef.Type) { $credDef.Type } else { 'Domain' }

                if ($credType -eq 'Local') {
                    # Local credential: derive OS flavor from Name or default to 'windows'
                    $flavor = 'windows'
                    if ($credDef.Name -match 'linux|unix|ssh') { $flavor = 'linux' }
                    elseif ($credDef.Name -match 'windows|winadmin|localadmin') { $flavor = 'windows' }
                    $key = "local:$flavor"
                    # Allow multiple local creds of same flavor (primary wins, fallbacks get suffix)
                    if ($localCreds.ContainsKey($key)) {
                        $i = 2
                        while ($localCreds.ContainsKey("${key}_${i}")) { $i++ }
                        $key = "${key}_${i}"
                    }
                    $localCreds[$key] = $cred
                    Write-Status $key "OK (Local)" "$($cred.UserName) -- $($credDef.Name)" Cyan
                }
                elseif ($credDef.DomainFQDN) {
                    $domain = $credDef.DomainFQDN.ToLower()
                    if ($credLookup.ContainsKey($domain)) {
                        # Secondary domain cred: give it a synthetic key
                        $i = 2
                        while ($credLookup.ContainsKey("${domain}_${i}")) { $i++ }
                        $domain = "${domain}_${i}"
                    }
                    $credLookup[$domain] = $cred
                    Write-Status $domain "OK (Domain)" "$($cred.UserName)" Green
                }
                else {
                    Write-Status ($credDef.Name) "SKIP" "Type=Domain but no DomainFQDN set" Yellow
                }
            }
        } else {
        ...
        # Manual -CredentialPaths hashtable: assume domain keys unless prefixed 'local:'
        foreach ($key in $CredentialPaths.Keys) {
            $path = $CredentialPaths[$key]
            if (Test-Path $path) {
                try {
                    $cred = Import-Clixml $path
                    if ($key -match '^local:') {
                        $localCreds[$key.ToLower()] = $cred
                        Write-Status $key "OK (Local)" "$($cred.UserName)" Cyan
                    } else {
                        $credLookup[$key.ToLower()] = $cred
                        Write-Status $key "OK (Domain)" "$($cred.UserName)" Green
                    }
                } catch {
                    Write-Status $key "FAIL" "Cannot load $path" Red
                }
            } else {
                Write-Status $key "MISSING" "$path not found" Yellow
            }
        }
    }

    if ($credLookup.Count -eq 0 -and $localCreds.Count -eq 0) {
        Write-Host "`n  No credentials loaded. Cannot continue." -ForegroundColor Red
        return
    }

    Write-Host "  Loaded $($credLookup.Count) domain credential(s) + $($localCreds.Count) local credential(s)" -ForegroundColor Gray

    # Set primary credential (ohdc.com or first available domain; local creds are never primary)
    $primaryCred = if ($credLookup.ContainsKey('ohdc.com')) { $credLookup['ohdc.com'] }
                   elseif ($credLookup.Count -gt 0) { $credLookup[($credLookup.Keys | Select-Object -First 1)] }
                   else {
                       Write-Host "  WARN: No domain credential available for Hyper-V host connection." -ForegroundColor Yellow
                       $localCreds[($localCreds.Keys | Select-Object -First 1)]
                   }
    ...
    try {
        $vms = Invoke-Command -ComputerName $HostName -Credential $primaryCred -ErrorAction Stop -ScriptBlock {
            ...
            foreach ($vm in $vmList) {
                ...
                # Pre-compute values that need try/catch (PS 5.1 cannot use try/catch inside @{})
                $guestOSValue = ''
                try { $guestOSValue = ($vm | Get-VMIntegrationService -ErrorAction SilentlyContinue | Where-Object { $_.Name -eq 'Guest Service Interface' }).PrimaryStatusDescription } catch {}
                $intSvcCount = 0
                try { $intSvcCount = @($vm | Get-VMIntegrationService -ErrorAction SilentlyContinue | Where-Object { $_.OperationalStatus -eq 'Ok' }).Count } catch {}

                [PSCustomObject]@{
                    Name          = $vm.Name
                    VMId          = $vm.Id.ToString()
                    State         = $vm.State.ToString()
                    GuestOS       = $guestOSValue
                    KVP           = $kvpData
                    NetworkAdapters = $nics
                    Generation    = $vm.Generation
                    IntSvcEnabled = $intSvcCount
                }
            }
    ...
    foreach ($vm in $vms) {
        ...
        # Priority order: (1) domain-match cred, (2) all other domain creds,
        # (3) local creds (Windows flavor for Windows guests, Linux for Linux)
        if ($result.WinningMethod -eq 'None' -and $vm.VMId) {
            ...
            $rotationQueue = @()
            # Domain creds first (excluding the one already tried as $vmCred)
            foreach ($credDomain in ($credLookup.Keys | Sort-Object)) {
                $rotationQueue += @{ 
                    Cred = $credLookup[$credDomain]
                    Source = $credDomain
                    Type = 'Domain' 
                }
            }
            # Local creds appended (tried only if all domain creds fail)
            # Prefer Windows local creds for Gen2 / non-Linux VMs; Linux for detected Linux
            $preferLinux = ($vm.GuestOS -match 'linux|ubuntu|centos|rhel|debian' -or
                            $vmTarget -match 'linux|ubuntu|centos|rhel|debian')
            $localKeys = @($localCreds.Keys | Sort-Object)
            if ($preferLinux) {
                $localKeys = @($localKeys | Where-Object { $_ -match 'linux' }) +
                             @($localKeys | Where-Object { $_ -notmatch 'linux' })
            } else {
                $localKeys = @($localKeys | Where-Object { $_ -match 'windows' }) +
                             @($localKeys | Where-Object { $_ -notmatch 'windows' })
            }
            foreach ($lk in $localKeys) {
                $rotationQueue += @{ 
                    Cred = $localCreds[$lk]
                    Source = $lk
                    Type = 'Local' 
            ...
            foreach ($entry in $rotationQueue) {
                $tryCred = $entry.Cred
                ...
                try {
                    ...
                    if ($psdResult2.Success) {
                        $result.PSDirect_MultiCred = "OK with $($tryCred.UserName) [$($entry.Type):$($entry.Source)] (guest: $($psdResult2.Data.Hostname))"
                        $result.WinningMethod = "PSDirect (VMBus) [$($entry.Type):$($entry.Source)]"
                        ...
                        $result.PSDirect_MultiCred = "FAIL with $($tryCred.UserName) [$($entry.Type)]: $($psdResult2.Error -replace '\r?\n.*','' -replace '.{60}$','')"
                    }
                } catch {
                    $result.PSDirect_MultiCred = "FAIL (host-level) with $($tryCred.UserName) [$($entry.Type)]"
                }
            }
