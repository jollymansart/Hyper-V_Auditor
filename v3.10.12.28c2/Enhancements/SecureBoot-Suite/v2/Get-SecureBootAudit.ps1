...
$activityLog    = Join-Path $OutputFolder "SecureBoot-Audit_${timestamp}.log"          # Structured timestamped log
$transcriptLog  = Join-Path $OutputFolder "SecureBoot-Audit_${timestamp}.transcript"   # Full console mirror

# ── Start transcript immediately so ALL console output is captured ──
# The transcript captures a verbatim mirror of everything printed to the console,
# including verbose messages, errors, and Write-Host output from remote jobs.
# It starts here, before any work runs, so nothing is lost.
try {
    Start-Transcript -Path $transcriptLog -Force | Out-Null
    Write-Host "[Transcript started: $transcriptLog]" -ForegroundColor DarkGray
}
catch {
...
function Write-AuditLog {
    # Writes a structured timestamped line to the ACTIVITY LOG (.log file)
    # and a colour-coded line to the console (which is separately captured by the transcript).
    # The two files serve different purposes:
    #   Activity log  -- machine-readable, parseable by grep/Select-String, no duplicate content
    #   Transcript    -- verbatim console mirror including verbose/error streams
    param([string]$Message, [string]$Level = 'Info')
    $ts   = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'
    $line = "[$ts][$Level] $Message"

    # Write structured line to activity log only (not via Write-Host to avoid transcript duplication)
    Add-Content -Path $activityLog -Value $line -Encoding UTF8

    # Write colour-coded line to console (transcript captures this separately)
    $color = switch ($Level) {
        'Error'   { 'Red'     }
        'Warning' { 'Yellow'  }
        'Success' { 'Green'   }
...
# Write activity log header (before any domain queries -- captures script invocation context)
Add-Content -Path $activityLog -Value "# ====================================================" -Encoding UTF8
Add-Content -Path $activityLog -Value "# SecureBoot Audit Activity Log" -Encoding UTF8
Add-Content -Path $activityLog -Value "# Started:    $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" -Encoding UTF8
Add-Content -Path $activityLog -Value "# RunAs:      $($env:USERDOMAIN)\$($env:USERNAME)" -Encoding UTF8
Add-Content -Path $activityLog -Value "# Machine:    $($env:COMPUTERNAME)" -Encoding UTF8
Add-Content -Path $activityLog -Value "# Script:     $($MyInvocation.MyCommand.Path)" -Encoding UTF8
Add-Content -Path $activityLog -Value "# Parameters: MaxParallel=$MaxParallel PingTimeoutMs=$PingTimeoutMs WinRMTimeoutSec=$WinRMTimeoutSec IncludeVMs=$IncludeVMs SkipPatchScan=$SkipPatchScan CertWarningDays=$CertWarningDays CertCriticalDays=$CertCriticalDays" -Encoding UTF8
Add-Content -Path $activityLog -Value "# Activity log: $activityLog" -Encoding UTF8
Add-Content -Path $activityLog -Value "# Transcript:   $transcriptLog" -Encoding UTF8
Add-Content -Path $activityLog -Value "# ====================================================" -Encoding UTF8
