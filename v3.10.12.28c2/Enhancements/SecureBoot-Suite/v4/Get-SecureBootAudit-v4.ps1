...
$certGuidance = @{
    ...
    'DB_MsWin' = @{
        Role   = 'Signature Database (db) -- Microsoft Windows Production PCA. The primary cert that authorizes all Microsoft Windows bootloaders: bootmgr.efi, winload.efi, and the Windows kernel. This cert is what actually allows Windows to boot under Secure Boot.'
        Action = 'This cert is typically current. If expired, Windows will not boot with Secure Boot enabled -- disable SB temporarily and apply Windows updates.'
        PostPatch = 'Updated periodically by Windows cumulative updates. If current cert is approaching expiry, a Windows Update will add a new one.'
    }
    'DBX' = @{
...
foreach ($row in $results) {
    ...
    foreach ($entry in $entries) {
        ...
        $certDetailRows.Add([PSCustomObject]@{
            ...
            ExpiryDate        = $expiry
            DaysRemaining     = $days
            CertAlertLevel    = if ($isRevoked) { 'Revoked' } elseif ($isHyperVPK -and $days -lt 0) { 'Expected-Expired (safe)' } else { $cal }
            IsRevoked         = $isRevoked
            IsHyperVStaticPK  = $isHyperVPK
            ActionNeeded      = -not $isHyperVPK -and ($isRevoked -or $cal -ne 'OK')
            CertRole          = $guide.Role
            ActionRequired    = if ($isRevoked) { 'CRITICAL: Revoked cert -- apply DBX update immediately' } else { $guide.Action }
